...
deployRouter.post('/:id/github-push', validate(githubPushSchema), async (_req, res) => {
  return res.status(501).json({ error: 'Not implemented', message: 'Git push not yet implemented in v2' });
});
...
deployRouter.post('/:id/store/configure', async (_req, res) => {
  return res.status(501).json({ error: 'Not implemented', message: 'Store configuration not yet implemented in v2' });
});

// POST /api/projects/:id/store/build
deployRouter.post('/:id/store/build', async (_req, res) => {
  return res.status(501).json({ error: 'Not implemented', message: 'Store build not yet implemented in v2' });
});

// POST /api/projects/:id/build-deploy
deployRouter.post('/:id/build-deploy', async (_req, res) => {
  return res.status(501).json({ error: 'Not implemented', message: 'Build + deploy not yet implemented in v2' });
});

// POST /api/projects/:id/github-actions
deployRouter.post('/:id/github-actions', async (_req, res) => {
  return res.status(501).json({ error: 'Not implemented', message: 'GitHub Actions not yet implemented in v2' });
});
